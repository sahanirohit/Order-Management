...
class Main:
    def __init__(self,root):
        ...
        def menu():
            ...
            mymenu.add_command(label="Pending Orders", command=pending_orders)
            mymenu.add_command(label="Completed Orders", command=completed_orders)
        ...
        def pending_orders():
            ...
            self.pending_order_table = ttk.Treeview(self.pending_table,columns=("date","partyname","status","orderno","designno","pick","mtr","wq","panno","d1","q1","c1","d2","q2","c2","d3","q3","c3","d4","q4","c4","d5","q5","c5","d6","q6","c6","d7","q7","c7","d8","q8","c8"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.txt_username = Entry(self.login,font=("times new roman",14),bg="#EEEEEE",bd=0)
        ...
        self.txt_password = Entry(self.login,font=("times new roman",14),bg="#EEEEEE",bd=0)
        ...
        login_btn = Button(self.login,text="Login",bd=0,command=lg,font=("times new roman",14,"bold"),fg="#141E27",bg="#42C2FF").place(x=240,y=200,width=100, height=25)
        ...
        def order_form(self):
            ...
            self.txt_date = Entry(self.order, width= 20, textvariable=self.date_var, bg="#EEEEEE",bd=0)
            ...
            # Row 11
            btn_insert = Button(self.order,text="INSERT",command=insert_order,font=("times new roman",8,"normal"),bd=0).place(x=20,y=660,width=80, height=30)
            
            btn_update = Button(self.order,text="UPDATE", command=update_order,font=("times new roman",8,"normal"),bd=0).place(x=135,y=660,width=80, height=30)
            
            btn_delete = Button(self.order,text="DELETE", command=delete_orders,font=("times new roman",8,"normal"),bd=0).place(x=245,y=660,width=80, height=30)
            
            btn_print = Button(self.order,text="PRINT",font=("times new roman",8,"normal"),bd=0).place(x=360,y=660,width=80, height=30)

            btn_clear = Button(self.order,text="CLEAR", command=clear_orders, font=("times new roman",8,"normal"),bd=0).place(x=485,y=660,width=80, height=30)
            ...
            combo_search = ttk.Combobox(self.data, textvariable=self.search_by, width=15, font=("times new roman",10,"normal"),state="readonly")
            combo_search['values']=("Select","PartyName","DesignNo","OrderNo","status")
            combo_search.current(0)
            combo_search.place(x=130,y=20)

            txt_search = Entry(self.data, textvariable=self.search_txt, width=27, bg="#EEEEEE",bd=1).place(x=290,y=20,height=20)
            btn_search = Button(self.data, text="Search", command=search_orders, bd=0).place(x=500,y=20,height=20,width=80)
            btn_showall = Button(self.data, text="Show All", command=fetch_orders, bd=0).place(x=620,y=20,height=20,width=80)

        
        # Edit Data
        # def get_cursor():
        #     curosor_row = self.order_table.focus()
        #     contents = self.order_table.item(curosor_row)
        #     row = contents['values']
